<template>
  <div class="order-train">
    <span class="order-train-main">{{ dailyTrainTicket.title }}</span>&nbsp;
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleQueryPassenger = () => {
      axios.get("/member/audience/query-mine").then((response) => {
    ...
    const finishCheckPassenger = () => {
      ...
      if (dailyTrainTicket.ticketLeft < tickets.value.length) {
        notification.error({description: '余票不足'});
        return;
      }
    ...
    const handleOk = () => {
      ...
      console.log("开始下单", JSON.stringify(dailyTrainTicket));

      const ticketListArr = ref([]);
      ticketListArr.value = tickets.value.map((item) => {
        return {
          audienceId: item.passengerId,
          audienceName: item.passengerName,
          concertId: dailyTrainTicket.id,
          seatNumber: 1
        }
      });
      ...
      axios.post("/business/confirm-order/do", {
        concertId: dailyTrainTicket.id,
        ticketLeft: dailyTrainTicket.ticketLeft,
        ticketList: ticketListArr.value,
